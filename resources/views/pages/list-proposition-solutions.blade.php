...
@section('subhead')
    <title>Liste des propositions des solutions</title>
@endsection
...
@section('subcontent')
    ...
    <div class="grid grid-cols-12 gap-6 mt-5">
        <div class="intro-y col-span-12 flex flex-wrap sm:flex-nowrap items-center mt-2">
            ...
            <button class="btn btn-primary shadow-md mr-2" data-tw-toggle="modal"
                data-tw-target="#new-order-modal-proposition">Ajouter une proposition de solution
            </button>
    ...
    <div class="intro-y col-span-12 overflow-auto lg:overflow-visible">
        ...
        <table class="table table-report -mt-2">
            ...
            <tbody>
                @forelse ($difficulteProjet->propositionSolutions as $solution)
                    <tr class="intro-x">
                        <td class="w-40">{{ $solution->id }}</td>
                        <td class="text-center">{{ $solution->description }}</td>
                        <td class="text-center">
                            <!-- Affichage du lien vers le fichier PDF si disponible -->
                            @if ($solution->file_path)
                                <a href="{{ asset('storage/' . $solution->file_path) }}" target="_blank"
                                    class="text-primary">
                                    Télécharger le PDF
                                </a>
                            @else
    ...
    <div id="new-order-modal-proposition" class="modal" tabindex="-1" aria-hidden="true">

        <div class="modal-dialog">
            <form id="proposition-form" method="POST" action="{{ route('difficultes.storeProposition') }}"
                enctype="multipart/form-data">
                <div class="modal-content">
                    <div class="modal-header">
                        <h2 class="font-medium text-base mr-auto">Ajouter une proposition de solution</h2>
                    </div>


                    <div class="modal-body grid grid-cols-12 gap-4 gap-y-3">
                        @csrf
                        <input type="hidden" name="difficulte_projet_id" value="{{ $difficulteProjet->id }}">
                        <!-- Description -->
                        <div class="col-span-12">
                            <label for="proposition-description" class="form-label">Description</label>
                            <textarea id="proposition-description" name="description" class="form-control flex-1 h-28"
                                placeholder="Entrez une description"></textarea>
                        </div>

                        <!-- Date -->
                        <div class="col-span-12">
                            <label for="proposition-date" class="form-label">Date</label>
                            <input id="proposition-date" name="date" type="date" class="form-control flex-1">
                        </div>
                        ...
                        <div class="col-span-12">
                            ...
                            <div class="flex items-center space-x-2">
                                ...
                                <button type="button" class="btn btn-outline-primary"
                                    onclick="document.getElementById('proposition-file').click()">Choisir un
                                    fichier</button>
    ...
    <script>
        ...
        document.getElementById('proposition-file').addEventListener('change', function() {
    ...
    <script>
        document.getElementById('proposition-form').addEventListener('submit', function(event) {
            const fileInput = document.getElementById('proposition-file');
    ...
    @if (session('successProposition'))
        <script>
            Swal.fire({
                icon: 'success',
                title: 'Proposition solution enregistré avec succès!',
